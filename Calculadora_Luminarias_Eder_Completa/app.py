...
st.markdown("Calcula el número de luminarias o los lux proporcionados en un espacio conforme a la norma. Ajusta reflectancias, CU, MF y consulta catálogos para mejor diseño.")
...
longitud = st.number_input("Longitud del área (m)", min_value=1.0)
ancho = st.number_input("Ancho del área (m)", min_value=1.0)
altura_montaje = st.number_input("Altura de montaje (m)", min_value=1.0)
altura_plano_trabajo = st.number_input("Altura del plano de trabajo (m)", min_value=0.5, value=0.75)
area = longitud * ancho

st.header("3. Parámetros de luminaria")
flujo_luminoso = st.number_input("Flujo luminoso por lámpara (lm)", min_value=100.0)
mf = st.slider("Índice de mantenimiento (MF)", 0.5, 1.0, 0.8, 0.01)

st.subheader("Reflectancias del entorno (para cálculo más realista)")
reflect_techo = st.slider("Reflectancia del techo (%)", 10, 90, 80, 5) / 100
reflect_muros = st.slider("Reflectancia de muros (%)", 10, 90, 70, 5) / 100
reflect_piso = st.slider("Reflectancia del piso (%)", 10, 90, 20, 5) / 100

def estimar_cu(rt, rm, rp):
    return round(0.25 + 0.5 * (rt + rm + 0.5 * rp) / 3, 2)

cu_auto = estimar_cu(reflect_techo, reflect_muros, reflect_piso)

modo_cu = st.radio("¿Cómo deseas definir el CU?", ["Automático (por reflectancias)", "Manual"])
if modo_cu == "Manual":
    cu = st.slider("Coeficiente de utilización (CU)", 0.3, 0.9, cu_auto, 0.01)
else:
    cu = cu_auto
    st.success(f"CU estimado automáticamente: {cu}")
...
datos_export = {
    ...
    "Reflectancia techo": reflect_techo,
    "Reflectancia muros": reflect_muros,
    "Reflectancia piso": reflect_piso,
    **resultado
}
